...
async function GAMES202Main() {
	...
	for (let i = 0; i < envmap.length; i++) {
		...
		let val = '';
		await this.loadShaderFile(envmap[i] + "/transport_shadowed.txt").then(result => {
			val = result;
		});
		...
		await this.loadShaderFile(envmap[i] + "/light_shadowed.txt").then(result => {
			val = result;
		});
	...
	function mainLoop(now) {
		...
		// console.log(guiParams.envmapId);

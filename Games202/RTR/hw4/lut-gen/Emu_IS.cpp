...
Vec3f ImportanceSampleGGX(Vec2f Xi, Vec3f N, float roughness) {
    ...
    const float theta = atan(a * sqrt(Xi.x) / sqrt(1.0f - Xi.x));
...
Vec3f IntegrateBRDF(Vec3f V, float roughness) {
    ...
    for (int i = 0; i < sample_count; i++) {
        ...
        const float weight = std::max(dot(L, H), 0.0f) * GeometrySmith(roughness, NoV, NoL) / (NoH * NoV);
        res += Vec3f(1.0f) * weight;
    ...
    // 1 - Eu
    // return Vec3f(1.0f) - res / sample_count;

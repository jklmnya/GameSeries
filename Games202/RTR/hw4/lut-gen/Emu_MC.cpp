...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
    ...
    for (int i = 0; i < sample_count; i++) {
      // TODO: To calculate (fr * ni) / p_o here
        ...
        const float G = GeometrySmith(roughness, dot(N, V), dot(N, sampleList.directions[i]));
        const float Ui = dot(N, sampleList.directions[i]);
        const float BRDF = F * D * G / (4.0f * dot(N, V) * dot(N, sampleList.directions[i]));
        const float res = BRDF * Ui / sampleList.PDFs[i];

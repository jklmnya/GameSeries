...
public:
	...
	inline void useUniformMat4(const char* valName, const glm::mat4& m4) const {
		glUniformMatrix4fv(glGetUniformLocation(shaderProgramID, valName), 1, GL_FALSE, glm::value_ptr(m4));
	}

	inline void useUniformVec3(const char* valName, const glm::vec3 v3) const {
		glUniform3f(glGetUniformLocation(shaderProgramID, valName), v3.x, v3.y, v3.z);
	}
};

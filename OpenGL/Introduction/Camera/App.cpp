...
int main()
{
	...
	shader.useUniformInt("texture1", 0);
	shader.useUniformInt("texture2", 1);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		shader.useUniformMat4("MatView", view);

		glm::mat4 projection = glm::mat4(1.0f);
		projection = glm::perspective(glm::radians(camera.getFOV()), (float)(SCR_WIDTH) / (float)(SCR_HEIGHT), 0.1f, 100.f);
		shader.useUniformMat4("MatProjection", projection);
		...
		for (int i = 0; i < 10; ++i) {
			...
			shader.useUniformMat4("MatModel", model);

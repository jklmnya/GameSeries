...
namespace NativeOpenGL {
	...
	enum FrameBufferType {
		FB_WITH_COLOR_DEPTH_LDR,
		FB_WITH_COLOR_DEPTH_HDR,
		FB_WITH_DEPTH
	};
	...
	std::tuple<unsigned int, unsigned int, unsigned int> GenFrameBufferHDR(const unsigned int Width, const unsigned int Height) {
		unsigned int FrameBuffer;
		glGenFramebuffers(1, &FrameBuffer);
		glBindFramebuffer(GL_FRAMEBUFFER, FrameBuffer);
		// Create Color Attachment Texture

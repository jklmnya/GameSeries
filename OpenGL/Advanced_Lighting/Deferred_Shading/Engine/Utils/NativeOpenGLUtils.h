...
namespace NativeOpenGL {
	...
	GBuffer GenGBuffer(const unsigned int Width, const unsigned int Height) {
		GBuffer gBuffer;
		glGenFramebuffers(1, &gBuffer.FrameBuffer);
		glBindFramebuffer(GL_FRAMEBUFFER, gBuffer.FrameBuffer);
		// Position Buffer
		glGenTextures(1, &gBuffer.PosBuffer);
		glBindTexture(GL_TEXTURE_2D, gBuffer.PosBuffer);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, Width, Height, 0, GL_RGB, GL_FLOAT, NULL);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, gBuffer.PosBuffer, 0);
		// Normal Buffer
		glGenTextures(1, &gBuffer.NormalBuffer);
		glBindTexture(GL_TEXTURE_2D, gBuffer.NormalBuffer);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, Width, Height, 0, GL_RGB, GL_FLOAT, NULL);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, gBuffer.NormalBuffer, 0);
		// Albedo + Specular Buffer
		glGenTextures(1, &gBuffer.AlbedoSpecBuffer);
		glBindTexture(GL_TEXTURE_2D, gBuffer.AlbedoSpecBuffer);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, Width, Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, GL_TEXTURE_2D, gBuffer.AlbedoSpecBuffer, 0);

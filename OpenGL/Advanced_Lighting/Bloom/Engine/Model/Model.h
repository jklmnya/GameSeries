...
public:
	...
	void Draw(Shader& shader) {
		...
		for (auto& texture : Textures) {
			...
			shader.useUniformInt(Name.c_str(), idx);
...
private:
	...
	std::vector<Texture> LoadSpecificTexture(aiMaterial* Material, aiTextureType aiTextureType_, 
		...
		for (unsigned int i = 0; i < Material->GetTextureCount(aiTextureType_); ++i) {
			...
			texture.TextureID = TextureUtil::TextureFromFile(FileName.c_str(), false);
...
public:
	PRTMesh(std::vector<PRTVertex> vertices, std::vector<unsigned int> indices) :
...
private:
	void SetupMesh() {
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(PRTVertex) * Vertices.size(), &Vertices[0], GL_STATIC_DRAW);

		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * Indices.size(), &Indices[0], GL_STATIC_DRAW);

		// Vertex Position
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(PRTVertex), (void*)0);
		glEnableVertexAttribArray(0);

		// Transfer
		glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(PRTVertex), (void*)(sizeof(float) * 3));
		glEnableVertexAttribArray(1);
		glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(PRTVertex), (void*)(sizeof(float) * 6));
		glEnableVertexAttribArray(2);
		glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, sizeof(PRTVertex), (void*)(sizeof(float) * 9));
		glEnableVertexAttribArray(3);
...
private:
	std::vector<PRTMesh> Meshes;
	std::vector<Mat3> LightTransferSH;
	int PRTIndex = 0;

public:
	PRTModel(const char* path, const char* PRTPath) {
		Logger::GetLogger()->PrintMessage(path);
		LightTransferSH = StringUtil::PRTLightTransferToSH(PRTPath);
		LoadModel(path);
	}
	...
	inline std::vector<PRTMesh>& GetModelMeshes() {
		return Meshes;
	}
...
private:
	...
	PRTMesh ProcessMesh(aiMesh* Mesh_, const aiScene* Scene) {
		std::vector<PRTVertex> Vertices;
		std::vector<unsigned int> Indices;
		// Process Mesh's Vertices
		for (unsigned int i = 0; i < Mesh_->mNumVertices; ++i) {
			PRTVertex vertex;
			vertex.Position = Vector3(Mesh_->mVertices[i].x, Mesh_->mVertices[i].y, Mesh_->mVertices[i].z);
			vertex.SHValue = LightTransferSH[PRTIndex++];
			Vertices.push_back(std::move(vertex));
		}
		// Process Mesh's Faces(Indices)
		for (unsigned int i = 0; i < Mesh_->mNumFaces; i++) {
		...
		return PRTMesh(std::move(Vertices), std::move(Indices));
	}

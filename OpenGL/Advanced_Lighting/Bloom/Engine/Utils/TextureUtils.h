...
namespace TextureUtil {
	...
	unsigned int LoadTexture(unsigned char* data, int width, int height, int nrComponents, bool bUseGammaCorrection) {
		...
		if (data) {
			...
			case 3:
				...
				internalFormat = bUseGammaCorrection ? GL_SRGB : GL_RGB;
				break;
			case 4:
				textureFormat = GL_RGBA;
				internalFormat = bUseGammaCorrection ? GL_SRGB_ALPHA : GL_RGBA;
				break;
			default:
				break;
			}
	...
	unsigned int TextureFromFile(const char* path, bool bUseGammaCorrection) {
		int width, height, nrComponents;
		unsigned char* data = stbi_load(path, &width, &height, &nrComponents, 0);
		return LoadTexture(data, width, height, nrComponents, bUseGammaCorrection);
	}

	unsigned int TextureFromFbx(const aiTexture* _aiTexture, bool bUseGammaCorrection) {
		int width, height, nrComponents;
